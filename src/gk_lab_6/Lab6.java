...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		float[] gray = { 0.03f, 0.03f, 0.03f, 0 };
		float[] zero = { 0, 0, 0, 0 };
		...
		float[] position2 = { 10,10,10,10 };
		float[] position3 = { 3,2,5,2};
		
		gl2.glLightfv(gl2.GL_LIGHT1, gl2.GL_POSITION, position,0);
		gl2.glLightfv(gl2.GL_LIGHT2, gl2.GL_POSITION, position2,0);
		gl2.glLightfv(gl2.GL_LIGHT4, gl2.GL_POSITION, position3,0);
		
		float[] blue1  = {2.0f, 1.0f, 1.0f, 15, 20, 30};
        float[] blue2  = {0,0f, 0.2f, 0.2f, 20, 20, 30};
        float[] blue3  = {0.01f, 0, 0, 20, 15, 20, 30};
		...
		gl2.glLightfv(gl2.GL_LIGHT4, gl2.GL_AMBIENT, blue1,0);
		gl2.glLightfv (gl2.GL_LIGHT4, gl2.GL_DIFFUSE, blue2,0);
		gl2.glLightfv(gl2.GL_LIGHT4, gl2.GL_AMBIENT, blue3,0);

		
		float ambientLevel[] = { 1f, 2f, 2f, 1f };
		gl2.glLightModelfv( GL2.GL_LIGHT_MODEL_AMBIENT, ambientLevel, 0);
		

		
		gl2.glMaterialfv (GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[8], 0); //chrome
		draw_stozek(gl2,3, 0, -1,-0);
		
		
	} // end display()
	private void draw_stozek(GL2 gl2,
							float wysokosc_stozka,
							float translacja_x,
							float translacja_y,
							float translacja_z) 
	{
		...
		{
		 draw_podstawa(promien,gl2,i,0,translacja_x,translacja_y,translacja_z);							
		 draw_podstawa(promien,gl2,i, wysokosc_stozka=10.0f,translacja_x,translacja_y,translacja_z);			
		}
		gl2.glEnd ();
	}
	private void draw_podstawa(int promien, GL2 gl2, int i, float wysokosc, float x, float y, float z) {
		
		gl2.glVertex3f((float)(promien*Math.cos(2*i*Math.PI /16))+x,0+y, (float)(promien*Math.sin(2*i*Math.PI /16)+z));
		
		gl2.glVertex3f((float)(promien*Math.cos(2*(i+1)*Math.PI /16))+x,0+y, (float)(promien*Math.sin(2*(i+1)*Math.PI /16)+z));
		
		gl2.glVertex3f(0+x,wysokosc+y*2, 0+z);
